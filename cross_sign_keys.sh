...
MASTER_NPUB=$(PIPEK1_SEC_KEY="$MASTER_SEC" "$PIPE_K1" pubkey | grep Npub | awk '{print $2}')
CHILD_NPUB=$(PIPEK1_SEC_KEY="$CHILD_SEC" "$PIPE_K1" pubkey | grep Npub | awk '{print $2}')
...
SIG_M2C=$(echo -n "$STATEMENT_M2C" | PIPEK1_SEC_KEY="$MASTER_SEC" "$PIPE_K1" sign | base64 -w 0)
...
SIG_C2M=$(echo -n "$STATEMENT_C2M" | PIPEK1_SEC_KEY="$CHILD_SEC" "$PIPE_K1" sign | base64 -w 0)
...
echo -n "$STATEMENT_M2C" | "$PIPE_K1" verify --sig <(echo "$SIG_M2C" | base64 -d) --pub "$MASTER_NPUB"
echo "  [OK] Master -> Child Signature Verified"

echo -n "$STATEMENT_C2M" | "$PIPE_K1" verify --sig <(echo "$SIG_C2M" | base64 -d) --pub "$CHILD_NPUB"
